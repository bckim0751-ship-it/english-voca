...
function loadProg(){
  try{return JSON.parse(localStorage.getItem('evProg'))||fresh();}
...
function dateStr(offset){const d=new Date();d.setDate(d.getDate()+offset);return d.getFullYear()+'-'+pad(d.getMonth()+1)+'-'+pad(d.getDate());}
...
function blankWord(sentence,word){
  ...
  // Try word with common business suffixes
  r=sentence.replace(new RegExp('\\b'+esc+'(?:s|es|d|ed|ing|tion|ment|ive|al|ity|ly)?\\b','gi'),'_____');
  if(r!==sentence)return r;
  // Fallback
  return sentence.replace(new RegExp(esc,'gi'),'_____');
}
...
function markWord(status){
  ...
  setTimeout(()=>{if(state.idx<state.todayWords.length-1){state.idx++;renderCard();}},250);
}
function updateRing(){
...
function buildQuiz(words){
  ...
  return words.map((w,i)=>{
    ...
    if(type==='fill'){
      ...
      return{w,type,sentence,hint:w.korean,opts,correct:w.word,correctIdx:opts.indexOf(w.word)};
    }else if(type==='en-ko'){
      const opts=seededShuffle([w.korean,...others.map(x=>x.korean)],w.id+i*13);
      return{w,type,q:w.word,opts,correct:w.korean,correctIdx:opts.indexOf(w.korean)};
    }else{
      const opts=seededShuffle([w.word,...others.map(x=>x.word)],w.id+i*13+100);
      return{w,type,q:w.korean,opts,correct:w.word,correctIdx:opts.indexOf(w.word)};
    }
  });
}
function renderQ(){
  ...
  const typeLabels={'en-ko':'영어 → 한국어','ko-en':'한국어 → 영어','fill':'빈칸 채우기'};
  document.getElementById('q-type').textContent=typeLabels[q.type];
  const qText=document.getElementById('q-text');
  if(q.type==='fill'){
    const blanked=q.sentence.replace('_____','<span class="blank">_____</span>');
    qText.innerHTML='<div class="q-hint">'+q.hint+'</div><div class="q-sentence">'+blanked+'</div>';
  }else{
...
function showResult(){
  ...
  const msgs=[[10,'완벽해요! 🎉','모든 문제를 맞혔어요!'],[8,'훌륭해요! 👏','거의 다 맞혔어요!'],[6,'잘 했어요! 😊','조금 더 연습해보세요.'],[0,'계속 노력해요! 💪','복습이 필요해요.']];
  const [,msg,sub]=msgs.find(([t])=>score>=t);
  document.getElementById('score-msg').textContent=msg;
  document.getElementById('score-sub').textContent=sub;
  const typeKr={'en-ko':'영→한','ko-en':'한→영','fill':'빈칸'};
  document.getElementById('answer-review').innerHTML=
    state.quizAnswers.map(a=>`<div class="ans-row"><span class="ans-word">${a.w.word}</span><span class="ans-type">${typeKr[a.type]||''}</span><span class="ans-korean">${a.w.korean}</span><span>${a.ok?'✅':'❌'}</span></div>`).join('');
}
